...
但是磁盘数据的拷贝往往就是针对多盘的场景，在多盘的场景下，没有冲突的“盘对”（Volume Pair）是可以独立进行操作的，只是在运行时发掘这样的并行特征并且能够低开销的调度有一定的难度。2.2的流水线Step中就存在可以并行执行的部分，我们的工作就是挖掘出这一系列Step中可以并行的部分，时刻做到最大的并行化，提高数据迁移的效率。
